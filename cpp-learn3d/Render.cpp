    ...
    const Mat& tmat)
{
    ...
    rot += 0.0005f;
    const Quat rotQuat = Quat::fromAxisRotation(Vec3(1.0, 0.0, 0.0), rot);
    ...
    const Vec3 tv1 = tmat * scMat * rotMat * v1;
    const Vec3 tv2 = tmat * scMat * rotMat * v2;
    const Vec3 tv3 = tmat * scMat * rotMat * v3;
...
void drawMesh(Canvas& canvas, const Camera& camera, const Mesh& mesh)
{
    ...
    const Mat worldMatrix = camera.getProjMatrix() * camera.getViewMatrix(); ///tMat * Mat::scaleMatrix(0.2, 0.2, 0.2) * Mat::identity();
